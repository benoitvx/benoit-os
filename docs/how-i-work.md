...
That's why several of the skills in this repo touch Obsidian directly:
- `save-article` for ingest (web articles → structured markdown in `Veille/`)
- `correction-transcription` for meetings — **I transcript every meeting** (Whisper + diarisation), and the skill cleans up the raw output: it resolves `SPEAKER_XX` to real names, fixes acronyms and proper nouns against my personal referentials, and emits a structured CR (Points clés / Décisions / Actions) on top of the corrected transcript. Mandatory step 4: list every <90% confidence item (speakers, proper nouns, intents) before any rewrite — anti-hallucination guardrail
- `ingerer-transcripts` chains the whole thing: detects new raw transcripts from collab notes (La Suite Docs, Notion…) + 3rd-party inboxes (Granola, Otter…), matches each to a calendar event via a read-only iCal feed, routes to `<meeting-notes>/<series>/SXX.md`, then hands off to `correction-transcription`
